...
interface BudgetWithDetails extends Omit<Budgets, 'maxAmount'> {
  ...
  totalSpent: number;
  free: number;
  latestTransactions: (Omit<Transactions, 'amount'> & { amount: number })[];
}
...
export const getBudgetService = async (userId: number) => {
  ...
  const budgetsWithDetails: BudgetWithDetails[] = await Promise.all(
    budgets.map(async (budget: Budgets) => {
      ...
      return {
        ...
        totalSpent,
        free,
        latestTransactions: latestTransactions.map((t: Transactions) => ({
          ...t,
  ...
  const totalSpentAllCategories = budgetsWithDetails.reduce(
    (sum: number, budget: BudgetWithDetails) => sum + budget.totalSpent, 0
  );

  const totalMaxAmount = budgetsWithDetails.reduce(
    (sum: number, budget: BudgetWithDetails) => sum + budget.maxAmount, 0
  );
